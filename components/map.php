...
<section id="network" class="section-padding bg-surface overflow-hidden" aria-labelledby="network-title">
            ...
            <div class="europe-map-container relative p-4 sm:p-8">
                <svg id="europe-map" viewBox="0 0 800 600" class="w-full h-auto" role="img" aria-label="European logistics network map">
                    <defs>
                        <linearGradient id="route-gradient" x1="0%" y1="0%" x2="100%" y2="0%">
                            <stop offset="0%" stop-color="#003399" stop-opacity="0.2"/>
                            <stop offset="50%" stop-color="#1A2B44" stop-opacity="0.6"/>
                            <stop offset="100%" stop-color="#003399" stop-opacity="0.2"/>
                        </linearGradient>
                        <filter id="glow">
                    ...
                    <path class="map-land" d="M120,180 L200,120 L280,100 L360,90 L420,100 L480,80 L540,90 L600,110 L650,140 L680,180 L700,220 L720,280 L710,340 L680,400 L640,450 L580,480 L520,500 L460,510 L400,500 L340,490 L280,470 L220,440 L180,400 L150,350 L130,300 L120,250 Z" fill="#F4F6F8" stroke="#E8EDF2" stroke-width="1"/>
                    ...
                    <g class="map-marker <?= $country['hub'] ? 'map-marker-hub' : '' ?>" data-country="<?= e($country['id']) ?>" tabindex="0" role="button" aria-label="<?= e(strtoupper($country['id'])) ?>">
                        <?php if ($country['hub']): ?>
                        <circle cx="<?= $country['cx'] ?>" cy="<?= $country['cy'] ?>" r="24" class="map-pulse" fill="#003399" opacity="0.15"/>
                        <circle cx="<?= $country['cx'] ?>" cy="<?= $country['cy'] ?>" r="16" class="map-pulse-delay" fill="#003399" opacity="0.1"/>
                        <?php endif; ?>
                        <circle cx="<?= $country['cx'] ?>" cy="<?= $country['cy'] ?>" r="<?= $country['hub'] ? '10' : '7' ?>" class="<?= $country['hub'] ? 'fill-eu-blue' : 'fill-primary' ?>" filter="url(#glow)"/>
                        <text x="<?= $country['cx'] ?>" y="<?= $country['cy'] + 28 ?>" text-anchor="middle" class="map-label fill-muted text-xs font-medium" font-size="11"><?= e($country['label']) ?></text>
                    </g>
                    <?php endforeach; ?>

                    <!-- Moving dots on routes -->
                    <circle class="route-dot" r="3" fill="#003399" filter="url(#glow)">
                ...
                <div class="map-legend mt-6 flex flex-wrap items-center justify-center gap-6 text-sm">
                    <div class="flex items-center gap-2">
                        <span class="w-3 h-3 rounded-full bg-eu-blue"></span>
                        <span class="text-muted text-sm"><?= e($t['map']['romania']) ?></span>
                    </div>
                    <div class="flex items-center gap-2">
                        <span class="w-8 h-0.5 bg-gradient-to-r from-eu-blue to-primary"></span>
                        <span class="text-muted text-sm"><?= e($t['map']['routes']) ?></span>
                    </div>
                </div>
            </div>
        </div>
